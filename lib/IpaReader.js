  ...
  this.getEntries(whatYouNeed, function (error, buffers) {
    ...
    if (that.options.withIcon) {
      if (typeof Blob === 'undefined' || typeof document === 'undefined' || typeof URL === 'undefined') {
        ...
        return that.getEntry(new RegExp(iconName.toLowerCase()), function (error, buffer) {
          if (error) return callback(error);

          if (buffer) {
            const CgBICleaner = new CgBICleaner(buffer)
            buffer = CgBICleaner.parse();
            if (that.options.iconType === 'base64') {
              plistInfo.icon = 'data:image/png;base64,' + buffer.toString('base64');
            }else{
              plistInfo.icon = buffer;
            }
          }
